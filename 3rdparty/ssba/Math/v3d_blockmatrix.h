...
namespace V3D
{
   ...
   struct DiagonalBlockMatrix
   {
         DiagonalBlockMatrix(int const M, int const N, int const P)
            : _M(M), _N(N), projection_matrix_(P)
         {
            _values = new Elem[_M*_N*projection_matrix_];
            //std::fill(_values, _values + _M*_N*projection_matrix_, Elem(0));
         }
         ...
         int const getP() const { return projection_matrix_; }

         int const num_rows() const { return _M*projection_matrix_; }
         int const num_cols() const { return _N*projection_matrix_; }

         template <typename Vec>
         void getBlock(int const m, int const n, Vec& dst) const
         {
            assert(dst.size() == projection_matrix_);

            int const ofs = this->getBlockOfs(m, n);
            for (int i = 0; i < projection_matrix_; ++i) dst[i] = _values[ofs + i];
         }
         ...
         Elem operator()(int const row1, int const col1) const
         {
            ...
            int const m = row / projection_matrix_;
            int const n = col / projection_matrix_;
            int const pr = row - m*projection_matrix_;
            int const pc = col - n*projection_matrix_;
         ...
         void augmentDiagonal(Elem const mu)
         {
            for (int k = 0; k < std::min(_M, _N); ++k)
            {
               ...
               for (int i = 0; i < projection_matrix_; ++i) _values[ofs + i] += mu;
            }
         } // end augmentDiagonal()
         ...
         void scaleRows(Vec const& w)
         {
            ...
            for (int m = 0; m < _M; ++m)
               for (int p = 0; p < projection_matrix_; ++p)
               {
                  Elem const s = w[m*projection_matrix_ + p];
                  for (int n = 0; n < _N; ++n)
         ...
         void scaleColumns(Vec const& w)
         {
            ...
            for (int n = 0; n < _N; ++n)
               for (int p = 0; p < projection_matrix_; ++p)
               {
                  Elem const s = w[n*projection_matrix_ + p];
                  for (int m = 0; m < _M; ++m)
                     this->getBlock(m, n)[p] *= s;
               } // end for (p)
         } // end scaleRows()
         ...
         {
            int const n = col / projection_matrix_;
            int const p = col - n*projection_matrix_;

            for (int m = 0; m < _M; ++m)
            {
               rows[m] = m*projection_matrix_ + p;
         ...
         {
            int const m = row / projection_matrix_;
            int const p = row - m*projection_matrix_;

            for (int n = 0; n < _N; ++n)
            {
               cols[n] = n*projection_matrix_ + p;
      ...
      protected:
         ...
         {
            return (m*_N + n)*projection_matrix_;
         }

         int const _M, _N, projection_matrix_;
         Elem    * _values;
   ...
   makeZeroMatrix(DiagonalBlockMatrix<Elem>& dst)
   {
      std::fill(dst._values, dst._values + dst._M*dst._N*dst.projection_matrix_, Elem(0));
   }
   ...
   addMatrices(DiagonalBlockMatrix<Elem> const& A, DiagonalBlockMatrix<Elem> const& B, DiagonalBlockMatrix<Elem>& dst)
   {
      assert(A.projection_matrix_ == B.projection_matrix_);
      assert(A.projection_matrix_ == dst.projection_matrix_);
      assert(A._M == B._M);
      assert(A._N == B._N);
      assert(A._M == dst._M);
      assert(A._N == dst._N);

      int const sz = A.projection_matrix_ * A._M * A._N;
      ...
      for (int i = 0; i < dst._M; ++i)
         for (int j = 0; j < dst._N; ++j)
         {
            ...
            for (int k = 0; k < A._N; ++k)
            {
               ...
               for (int p = 0; p < A.projection_matrix_; ++p) valsC[p] += valsA[p] * valsB[p];
            } // end for (k)
         } // end for (j)
   } // end multiply_A_B()
      ...
      for (int i = 0; i < A._N; ++i)
         for (int j = 0; j < A._N; ++j)
         {
            ...
            for (int k = 0; k < A._M; ++k)
            {
               ...
               for (int p = 0; p < A.projection_matrix_; ++p) valsC[p] += valsAt[p] * valsA[p];
            } // end for (k)
         } // end for (j)
   } // end multiply_At_A()
