...
#define NLSQ_MAX_PARAM_TYPES 32
   ...
   {
         NLSQ_Residuals(std::vector<int> const& usedParamTypes, int const nMeasurements,
         ...
         std::vector<MatrixArray<double> * > _Js;
   ...
   struct NLSQ_LM_BlockHessian
   {
         ...
         std::vector<pair<int, int> > nonzeroPairs[NLSQ_MAX_PARAM_TYPES][NLSQ_MAX_PARAM_TYPES];
   ...
   struct NLSQ_CostFunction
   {
      ...
      protected:
		 ...
		 void fillAllJacobians(Vector<double> const& weights, std::vector<MatrixArray<double> * >& Js) const;
   ...
   struct NLSQ_LM_Optimizer : public LevenbergOptimizerCommon
   {
      ...
      protected:
		 ...
		 std::vector<pair<int, int> > _paramIdInverseMap; //!< the reverse mapping from global ids to (paramType,id) pairs.

		 std::vector<NLSQ_Residuals *>   _residuals;      // one per cost function
         //!< _hessianIndices establishes the link between the local hessian in a cost function to the global non-zero block.
		 std::vector<MatrixArray<int> *> _hessianIndices; // one per cost function
