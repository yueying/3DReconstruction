...
namespace V3D
{
   ...
   struct CameraMatrix
   {
         CameraMatrix()
         {
            makeIdentityMatrix(camera_matrix_);
            makeIdentityMatrix(rotation_matrix_);
            makeZeroVector(_T);
         ...
         {
            makeIdentityMatrix(camera_matrix_);
            camera_matrix_[0][0] = f;
            camera_matrix_[1][1] = f;
         ...
         CameraMatrix(Matrix3x3d const& K,
                      Matrix3x3d const& R,
                      Vector3d const& T)
            : camera_matrix_(K), rotation_matrix_(R), _T(T)
         {
            this->updateCachedValues(true, true);
         }
         ...
         CameraMatrix(Matrix3x3d const& K,
            ...
            : camera_matrix_(K)
         {
             rotation_matrix_ = RT.slice<3,3>(0,0);
             _T = RT.col(3);
            this->updateCachedValues(true, true);
         }
         ...
         {
             camera_matrix_ = cam.getIntrinsic();
             rotation_matrix_ = cam.getRotation();
         ...
         void setIntrinsic(Matrix3x3d const& K) { camera_matrix_ = K; this->updateCachedValues(true, false); }
         void setRotation(Matrix3x3d const& R) { rotation_matrix_ = R; this->updateCachedValues(false, true); }
         void setTranslation(Vector3d const& T) { _T = T; this->updateCachedValues(false, true); }

         void setCameraCenter(Vector3d const& c) { this->setTranslation(-1.0 * (rotation_matrix_ * c)); }

         template <typename Mat>
         void setOrientation(Mat const& RT)
         {
            rotation_matrix_[0][0] = RT[0][0]; rotation_matrix_[0][1] = RT[0][1]; rotation_matrix_[0][2] = RT[0][2];
            rotation_matrix_[1][0] = RT[1][0]; rotation_matrix_[1][1] = RT[1][1]; rotation_matrix_[1][2] = RT[1][2];
            rotation_matrix_[2][0] = RT[2][0]; rotation_matrix_[2][1] = RT[2][1]; rotation_matrix_[2][2] = RT[2][2];
            _T[0]    = RT[0][3]; _T[1]    = RT[1][3]; _T[2]    = RT[2][3];
            this->updateCachedValues(false, true);
         }

         Matrix3x3d const& getIntrinsic()   const { return camera_matrix_; }
         Matrix3x3d const& getRotation()    const { return rotation_matrix_; }
         Vector3d   const& getTranslation() const { return _T; }

         Matrix3x4d getExtrinsic() const
         {
            Matrix3x4d RT;
            copyMatrixSlice(rotation_matrix_,0,0,3,3,RT,0,0);
            RT[0][3] = _T[0];
            RT[1][3] = _T[1];
            RT[2][3] = _T[2];
            return RT;
         ...
         {
            Matrix3x4d RT;
            RT[0][0] = rotation_matrix_[0][0]; RT[0][1] = rotation_matrix_[0][1]; RT[0][2] = rotation_matrix_[0][2];
            RT[1][0] = rotation_matrix_[1][0]; RT[1][1] = rotation_matrix_[1][1]; RT[1][2] = rotation_matrix_[1][2];
            RT[2][0] = rotation_matrix_[2][0]; RT[2][1] = rotation_matrix_[2][1]; RT[2][2] = rotation_matrix_[2][2];
         ...
         Matrix3x4d getProjection() const
         {
            ...
            return camera_matrix_ * RT;
         }

         double getFocalLength() const { return camera_matrix_[0][0]; }
         double getAspectRatio() const { return camera_matrix_[1][1] / camera_matrix_[0][0]; }

         Vector2d getPrincipalPoint() const
         {
            Vector2d pp;
            pp[0] = camera_matrix_[0][2];
            pp[1] = camera_matrix_[1][2];
            return pp;
         }

         Vector2d projectPoint(Vector3d const& X) const
         {
            Vector3d q = camera_matrix_*(rotation_matrix_*X + _T);
            Vector2d res;
         ...
         {
            Vector3d XX = rotation_matrix_*X + _T;
            Vector2d p;
            p[0] = XX[0] / XX[2];
            p[1] = XX[1] / XX[2];
            p = distortion(p);
            ...
            Vector2d res;
            res[0] = camera_matrix_[0][0] * p[0] + camera_matrix_[0][1] * p[1] + camera_matrix_[0][2];
            res[1] =                   camera_matrix_[1][1] * p[1] + camera_matrix_[1][2];
            return res;
         }
         template<typename T>
         ...
         {
            return rotation_matrix_*dir;
         }
         template<typename T>
         InlineVector<T,2> transformPointIntoNormalizedCoordinate(InlineVector<T,2> const& p) const
         {
            return InlineVector<T,2>(camera_matrix_[0][0] * p[0] + camera_matrix_[0][1] * p[1] + camera_matrix_[0][2],camera_matrix_[1][1] * p[1] + camera_matrix_[1][2]);
         }
         template<typename T>
         ...
         void serialize(Archive& ar)
         {
            ...
            ar & camera_matrix_ & rotation_matrix_ & _T;
            if (ar.isLoading())
               this->updateCachedValues(true, true);
         }
      ...
      protected:
         ...
         {
            if (intrinsic) _invK = invertedMatrix(camera_matrix_);

            if (orientation)
            {
               makeTransposedMatrix(rotation_matrix_, _Rt);
               _center = _Rt * (-1.0 * _T);
            }
         }

         Matrix3x3d camera_matrix_, rotation_matrix_;
         Vector3d   _T;
