...
namespace V3D
{
   ...
   struct MetricBundleOptimizerBase : public SparseLevenbergOptimizer
   {
         ...
         MetricBundleOptimizerBase(double inlierThreshold,
			 std::vector<CameraMatrix>& cams,
			 std::vector<Vector3d>& Xs,
			 std::vector<Vector2d> const& measurements,
			 std::vector<int> const& corrspondingView,
			 std::vector<int> const& corrspondingPoint,
      ...
      protected:
		 ...
		 std::vector<CameraMatrix>& _cams;
		 std::vector<Vector3d>&     _Xs;

		 std::vector<Vector2d> const& _measurements;

		 std::vector<Vector3d>   _savedTranslations;
		 std::vector<Matrix3x3d> _savedRotations;
		 std::vector<Vector3d>   _savedXs;
   ...
   struct CommonInternalsMetricBundleOptimizer : public MetricBundleOptimizerBase
   {
         ...
         CommonInternalsMetricBundleOptimizer(int mode,
            ...
            : MetricBundleOptimizerBase(inlierThreshold, cams, Xs, measurements,
              ...
              _mode(mode), camera_matrix_(K), _distortion(distortion)
         {
         ...
         virtual void restoreAllParameters()
         {
            ...
            camera_matrix_ = _savedK;
            _distortion = _savedDistortion;
         }

      protected:
         int                    _mode;
         Matrix3x3d&            camera_matrix_;
...
//----------------------------------------------------------------------
   ...
   {
	   ScopedBundleExtrinsicNormalizer(std::vector<CameraMatrix>& cameras,
		   std::vector<Vector3d>& Xs)
            : _cameras(cameras), _Xs(Xs)
         {
			...
			std::vector<Vector3d> camCenters(N);
      ...
      protected:
		  std::vector<CameraMatrix>& _cameras;
		  std::vector<Vector3d>&     _Xs;
   ...
   {
	   ScopedBundleIntrinsicNormalizer(std::vector<CameraMatrix>& cameras,
      ...
      protected:
		  std::vector<CameraMatrix>& _cameras;
		  std::vector<Vector2d>&     _measurements;
		  std::vector<int> const&    _correspondingView;

		  std::vector<double>        _savedFs;
