...
#  These libs are linked against mvg_hwdrivers only (in shared libs,
#  in static all user apps will have to link against this)
# ====================================================================
SET(CMAKE_MVG_HAS_FFMPEG 0)
SET(CMAKE_MVG_HAS_FFMPEG_SYSTEM 0)
SET(MVG_FFMPEG_LIBS_TO_LINK "")
...
IF(PKG_CONFIG_FOUND)
	...
	IF(LIBAVCODEC_FOUND AND LIBAVUTIL_FOUND AND LIBAVFORMAT_FOUND AND LIBSWSCALE_FOUND)
		SET(CMAKE_MVG_HAS_FFMPEG 1)
		SET(CMAKE_MVG_HAS_FFMPEG_SYSTEM 1)
		...
		SET(MVG_FFMPEG_LIBS_TO_LINK "${MVG_FFMPEG_LIBS_TO_LINK}" "${LIBAVCODEC_LIBRARIES}" "${LIBAVFORMAT_LIBRARIES}" "${LIBAVUTIL_LIBRARIES}" "${LIBSWSCALE_LIBRARIES}")

		IF($ENV{VERBOSE})
			MESSAGE(STATUS " ffmpeg libs: ${MVG_FFMPEG_LIBS_TO_LINK}")
		ENDIF($ENV{VERBOSE})
	ENDIF(LIBAVCODEC_FOUND AND LIBAVUTIL_FOUND AND LIBAVFORMAT_FOUND AND LIBSWSCALE_FOUND)
ENDIF(PKG_CONFIG_FOUND)

IF(MSVC)
	SET( MVG_HAS_FFMPEG_WIN32 OFF CACHE BOOL "Add support for IP cameras and all FFmpeg-capable video sources")
ENDIF(MSVC)

IF(MVG_HAS_FFMPEG_WIN32)
	...
	SET(MVG_FFMPEG_LIBS_TO_LINK ${MVG_FFMPEG_LIBS_TO_LINK} "${FFMPEG_WIN32_AVCODEC_LIB}" "${FFMPEG_WIN32_AVUTIL_LIB}" "${FFMPEG_WIN32_AVFORMAT_LIB}" "${FFMPEG_WIN32_SWSCALE_LIB}")
ENDIF(MVG_HAS_FFMPEG_WIN32)
