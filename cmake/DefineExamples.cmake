...
IF(BUILD_EXAMPLES)
	SET(CMAKE_MVG_EXAMPLES_BASE_DIRECTORY "${CMAKE_SOURCE_DIR}/samples/")
	# Fix "\" --> "\\" for windows:
	string(REPLACE "\\" "\\\\" CMAKE_MVG_EXAMPLES_BASE_DIRECTORY ${CMAKE_MVG_EXAMPLES_BASE_DIRECTORY})

	#MESSAGE(STATUS "Parsing 'examples_config.h.in'")
	CONFIGURE_FILE("${CMAKE_SOURCE_DIR}/parse-files/examples_config.h.in" "${MVG_CONFIG_FILE_INCLUDE_DIR}/mvg/examples_config.h")
	...
	MACRO(GENERATE_CMAKE_FILES_SAMPLES_DIRECTORY)
		...
		#          "mvg_xxx mvg_yyy" -> "xxx yyy"
		SET(CMAKE_EXAMPLE_DEPS_STRIP "")
		FOREACH(DEP ${CMAKE_EXAMPLE_DEPS})
			# Only for "mvg_XXX" libs:
			STRING(REGEX REPLACE "mvg_(.*)" "\\1" STRIP_DEP ${DEP})
			IF(NOT "${STRIP_DEP}" STREQUAL "")
				LIST(APPEND CMAKE_EXAMPLE_DEPS_STRIP ${STRIP_DEP})
			ENDIF(NOT "${STRIP_DEP}" STREQUAL "")
		ENDFOREACH(DEP)

		FOREACH(CMAKE_MVG_EXAMPLE_NAME ${LIST_EXAMPLES_IN_THIS_DIR})
			#MESSAGE(STATUS "Example: ${CMAKE_MVG_EXAMPLE_NAME}")
			# Generate project file:
			CONFIGURE_FILE(${CMAKE_SOURCE_DIR}/samples/CMakeLists_template.txt.in ${CMAKE_SOURCE_DIR}/samples/${CMAKE_MVG_EXAMPLE_NAME}/CMakeLists.txt @ONLY)
			SET(CMAKE_COMMANDS_INCLUDE_EXAMPLE_DIRS_ROOT "${CMAKE_COMMANDS_INCLUDE_EXAMPLE_DIRS_ROOT}\nadd_subdirectory(${CMAKE_MVG_EXAMPLE_NAME})")
		ENDFOREACH(CMAKE_MVG_EXAMPLE_NAME)
	...
	SET(CMAKE_EXAMPLE_DEPS mvg_base)
	SET(CMAKE_EXAMPLE_LINK_LIBS ${MVG_LINKER_LIBS})
	...
	SET(CMAKE_EXAMPLE_DEPS mvg_base mvg_image)
	SET(CMAKE_EXAMPLE_LINK_LIBS ${MVG_LINKER_LIBS})
	GENERATE_CMAKE_FILES_SAMPLES_DIRECTORY()
