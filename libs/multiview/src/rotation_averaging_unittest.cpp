...
TEST ( rotation_averaging, ClosestSVDRotationMatrix )
{
  ...
  Mat3 Approximative_rotx = multiview::l2::ClosestSVDRotationMatrix(rotx);
...
TEST ( rotation_averaging, ClosestSVDRotationMatrixNoisy )
{
  ...
  // Check the Frobenius distance between the approximated rot matrix and the GT
  EXPECT_TRUE( FrobeniusDistance( rotx, Approximative_rotx) < 0.02);
  // Check that the Matrix is a rotation matrix (determinant == 1)
  EXPECT_NEAR( 1.0, Approximative_rotx.determinant(), 1e-8);
}
...
TEST ( rotation_averaging, RefineRotationsAvgL1IRLS_CompleteGraph)
{
  ...
  for (size_t i = 0; i < iNviews; ++i)
  {
    ...
    Mat3 Rrel;
    Vec3 trel;
    RelativeCameraMotion(d.rotation_matrix_[index0], d.translation_vector_[index0], d.rotation_matrix_[index1], d.translation_vector_[index1], &Rrel, &trel);
    vec_relativeRotEstimate.push_back(RelRotationData(index0, index1, Rrel, 1));

    RelativeCameraMotion(d.rotation_matrix_[index1], d.translation_vector_[index1], d.rotation_matrix_[index2], d.translation_vector_[index2], &Rrel, &trel);
    vec_relativeRotEstimate.push_back(RelRotationData(index1, index2, Rrel, 1));

    RelativeCameraMotion(d.rotation_matrix_[index0], d.translation_vector_[index0], d.rotation_matrix_[index2], d.translation_vector_[index2], &Rrel, &trel);
  ...
  if ( SIGN(vec_globalR[0](0,0)) != SIGN( d.rotation_matrix_[0](0,0) ))
  {
    Mat3 Rrel;
    Vec3 trel;
    RelativeCameraMotion(vec_globalR[0], Vec3::Zero(), d.rotation_matrix_[0], Vec3::Zero(), &Rrel, &trel);
    for (size_t i = 0; i < iNviews; ++i)
      vec_globalR[i] *= Rrel;
  }
  ...
  for (size_t i = 0; i < iNviews; ++i)
  {
    EXPECT_NEAR(0.0, FrobeniusDistance(d.rotation_matrix_[i], vec_globalR[i]), 1e-8);
  }
...
TEST ( rotation_averaging, RefineRotationsAvgL1IRLS_CompleteGraph_outliers)
{
  ...
  //- Solve the global rotation estimation problem :
  Matrix3x3Arr vec_globalR(iNviews);
  vec_globalR = d.rotation_matrix_;
  size_t nMainViewID = 0;
  ...
  // Check inlier list
  EXPECT_TRUE(std::accumulate(vec_inliers.begin(), vec_inliers.end(), 0) == 8);
  // Check outlier have been found
  EXPECT_TRUE(vec_inliers[0] == 0);
  EXPECT_TRUE(vec_inliers[3] == 0);
