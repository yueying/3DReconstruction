...
namespace fblib {
	namespace multiview {
		namespace resection {
			...
			void BuildActionMatrix(Matrix & A, const Mat &point_2d, const Mat &XPoints)  {

				const size_t n = point_2d.cols();
				for (size_t i = 0; i < n; ++i) {
					size_t row_index = i * 2;
					const Vec3 & X = XPoints.col(i);
					const Vec2 & x = point_2d.col(i);
					A(row_index, 0) = X(0);
					A(row_index, 1) = X(1);
					A(row_index, 2) = X(2);
					A(row_index, 3) = 1.0;
				...
				bool bcheck)
			{
				...
				translationMatrix << 1, 0, 0, vecTranslation(0),
					...
					0, 0, 0, 1;
				Mat3X XPoints;
				translate(point_3d, vecTranslation, &XPoints);
				...
				if (n == 6) {
					...
					BuildActionMatrix(A, point_2d, XPoints);
				...
				if (bcheck) {
					if (ratio > 1e-5) //Assert that at least only one solution if found by SVD
					{
						...
						size_t cpt = 0;
						for (size_t i = 0; i < n; ++i) {
							cpt += (Depth(R, t, point_3d.col(i)) > 0) ? 1 : 0;
						}
						if (cpt == n) {
							Ps->push_back(P);
						}
