#ifndef FBLIB_LINFINITY_COMPUTER_VISION_TRANSLATIONANDSTRUCTUREFrom_xi_RI_H_
#define FBLIB_LINFINITY_COMPUTER_VISION_TRANSLATIONANDSTRUCTUREFrom_xi_RI_H_
...
namespace fblib   {
	namespace sfm  {
		...
		void EncodeTiXi(const Mat & M, //Scene representation
			...
			RSparseMat & A, Vec & C,
			std::vector<LP_Constraints::LP_Sign> & vec_sign,
...
# define XVAR(j, el) (pointStart + 3*(j) + (el))
			...
			std::fill(vec_bounds.begin(), vec_bounds.end(), std::make_pair((double)-1e+30, (double)1e+30));
		...
		struct Translation_Structure_L1_ConstraintBuilder
		{
			...
			bool Build(double gamma, LP_Constraints_Sparse & constraint)
			{
				EncodeTiXi(_M, _vec_Ri,
					...
					constraint.vec_constrained_type_,
					constraint.objective_function_coeff_,
				...
				constraint.parameter_num_ = (Ncam + N3D) * 3;
...
#endif // FBLIB_LINFINITY_COMPUTER_VISION_TRANSLATIONANDSTRUCTUREFrom_xi_RI_H_
