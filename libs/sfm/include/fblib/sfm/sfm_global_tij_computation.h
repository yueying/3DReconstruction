#ifndef FBLIB_GLOBAL_SFM_ENGINE_TIJ_COMPUTATION_H
#define FBLIB_GLOBAL_SFM_ENGINE_TIJ_COMPUTATION_H
...
namespace fblib{
	namespace sfm{
		...
		bool estimate_T_triplet(
			size_t w, size_t h,
			const fblib::tracking::MapTracks & map_tracksCommon,
			const std::map<size_t, std::vector<fblib::feature::ScalePointFeature> > & map_feats,
			...
			std::vector<size_t> & vec_inliers)
		{
			...
			for (MapTracks::const_iterator iterTracks = map_tracksCommon.begin();
				...
				for (SubmapTrack::const_iterator iter = subTrack.begin(); iter != subTrack.end(); ++iter, ++index) {
					...
					const fblib::feature::ScalePointFeature & pt = map_feats.find(imaIndex)->second[featIndex];
			...
			if (!bTest)
			{
				FBLIB_INFO << "Triplet rejected : AC: " << dPrecision
					<< " median: " << median
...
#pragma omp parallel for schedule(static, 6)
			...
			for (int k = 0; k < vec_edges.size(); ++k)
			{
				...
				{
					FBLIB_INFO << "EDGES WAS PREVIOUSLY COMPUTED" << std::endl;
					continue;
				}
				...
				std::vector< fblib::utils::SortIndexPacketDescend < size_t, size_t> > packet_vec(vec_commonTracksPerTriplets.size());
				fblib::utils::SortIndexHelper(packet_vec, &vec_commonTracksPerTriplets[0]);
				...
				for (size_t i = 0; i < vec_possibleTriplets.size(); ++i)
				{
					...
					const size_t I = triplet.i, J = triplet.j, K = triplet.k;
					{
						...
						{
							FBLIB_INFO << dPrecision << "\t" << vec_inliers.size() << std::endl;
...
#endif // FBLIB_GLOBAL_SFM_ENGINE_TIJ_COMPUTATION_H
