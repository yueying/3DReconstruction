#ifndef MVG_SFM_INDEXED_IMAGE_GRAPH_EXPORT_H_
#define MVG_SFM_INDEXED_IMAGE_GRAPH_EXPORT_H_
...
namespace mvg  {
	namespace sfm  {
			...
			std::ostream & os)
		{
			...
			std::map< std::pair<size_t, size_t>, size_t > map_arcs;
			...
			for (std::map< std::pair<size_t, size_t>, size_t >::const_iterator iter = map_arcs.begin();
				iter != map_arcs.end();
				++iter)
			{
		...
		void exportToGraphvizData(const std::string& sfile, const GraphT & graph){
			...
			exportToGraphvizFormat_Nodal(graph, file);
			file.close();
...
#endif // MVG_SFM_INDEXED_IMAGE_GRAPH_EXPORT_H_
