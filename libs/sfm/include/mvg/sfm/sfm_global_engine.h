#ifndef MVG_SFM_GLOBAL_SFM_ENGINE_H
#define MVG_SFM_GLOBAL_SFM_ENGINE_H
	...
	namespace sfm{
		...
		public:
			...
			const std::vector< std::pair<size_t, size_t> > getImagesSize() const
			{
				...
				for (std::vector<mvg::feature::CameraInfo>::const_iterator iter_camera_info = vec_camera_image_names_.begin();
			...
			typedef std::map<size_t, mvg::camera::PinholeCamera > MapCamera;
		...
		private:
			...
			std::vector<mvg::feature::CameraInfo> vec_camera_image_names_;//!<图片名及对应id
			std::vector<mvg::feature::IntrinsicCameraInfo> vec_intrinsic_groups_;//!<图片对应的相机内参
			std::map< size_t, std::vector<mvg::feature::ScalePointFeature> > map_features_; //!< 图片对应的特征
			...
			std::shared_ptr<mvg::utils::HtmlDocumentStream> html_doc_stream_;
...
#endif // MVG_SFM_GLOBAL_SFM_ENGINE_H
