...
namespace mvg {
	namespace tracking{
		...
		template<typename T>
		bool InBounds(const Image<float> &image,
			const T &x,
			const T &y) {
		...
		bool AllInBounds(const Image<float> &image,
			const double *x,
			const double *y) {
			for (int i = 0; i < 4; ++i) {
		...
		template<typename T>
		static T SampleWithDerivative(const Image<RGBfColor> &image_and_gradient,
			const T &x,
			const T &y) {
			...
			if (JetOps<T>::IsScalar()) {
				...
				SampleLinear(image_and_gradient, scalar_y, scalar_x);
			}
			T xy[2] = { x, y };
		...
		public:
			PixelDifferenceCostFunctor(const TrackRegionOptions &options,
				const Image<RGBfColor> &image_and_gradient1,
			...
			void ComputeCanonicalPatchAndNormalizer() {
				...
				for (int r = 0; r < num_samples_y_; ++r) {
					for (int c = 0; c < num_samples_x_; ++c) {
						...
						pattern_positions_(r, c) = image_position(0);
						pattern_positions_(r, c) = image_position(1);
						...
						src_mean_ += pattern_and_gradient_(r, c) * mask_value;
			...
			bool operator()(const T *warp_parameters, T *residuals) const {
				...
				for (int r = 0; r < num_samples_y_; ++r) {
					for (int c = 0; c < num_samples_x_; ++c) {
						...
						Vec2 image1_position(pattern_positions_(r, c),
							pattern_positions_(r, c));
		...
		private:
			const TrackRegionOptions &options_;
			const Image<RGBfColor> &image_and_gradient1_;//这边是三通道
			const Image<RGBfColor> &image_and_gradient2_;//这边是三通道
			const Mat3 &canonical_to_image1_;
			int num_samples_x_;
			int num_samples_y_;
			const Warp &warp_;
			double src_mean_;
			Image<float> pattern_and_gradient_;
			...
			Image<float> pattern_positions_;

			Image<float> pattern_mask_;
		};
		...
		bool SearchAreaTooBigForDescent(const Image<float> &image2,
			const double *x2, const double *y2) {
		...
		// This makes it possible to map between Eigen float arrays and Image<RGBfColor>
		...
		void CreateBrutePattern(const double *x1, const double *y1,
			const double *x2, const double *y2,
			const Image<RGBfColor> &image1,
			const Image<float> *image1_mask,
			FloatArray *pattern,
			FloatArray *mask,
			int *origin_x,
			int *origin_y) {
			...
			for (int r = min_y; r < max_y; ++r) {
				for (int c = min_x; c < max_x; ++c) {
					...
					if (PointInQuad(x1, y1, src_x, src_y)) {
						(*pattern)(i, j) = SampleLinear(image1, src_y, src_x)(0);
						(*mask)(i, j) = 1.0;
						if (image1_mask) {
		...
		template<typename Warp>
		bool BruteTranslationOnlyInitialize(const Image<RGBfColor> &image1,
			const Image<float> *image1_mask,
			const Image<float> &image2,
			...
			Map<const FloatArray> search(image2.GetMat().data(), image2.Height(), image2.Width());
		...
		template<typename Warp>
		void TemplatedTrackRegion(const Image<float> &image1,
			const Image<float> &image2,
			const double *x1, const double *y1,
...
#undef HANDLE_MODE
		...
		bool SamplePlanarPatch(const Image<float> &image,
			const double *xs, const double *ys,
			int num_samples_x, int num_samples_y,
			Image<float> *mask, Image<float> *patch,
