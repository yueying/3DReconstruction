...
namespace mvg {
	namespace tracking{
		...
		static bool RegionIsInBounds(const Image<float> &image1,
			...
			if (min_x < 0.0 ||
				min_y < 0.0) {
				notify(INFO) << "Out of bounds; min_x: " << min_x << ", min_y: " << min_y;
				return false;
			}
			...
			if (max_x > image1.cols() ||
				max_y > image1.rows()) {
				notify(INFO) << "Out of bounds; max_x: " << max_x << ", max_y: " << max_y
		...
		bool EsmRegionTracker::Track(const Image<float> &image1,
			...
			*y2 = yy2[0] + half_window_size;
			...
			Image<RGBfColor> image_and_gradient2_sampled[2];
			...
			for (int r = 0; r < width; ++r) {
				for (int c = 0; c < width; ++c) {
					float e = image_and_gradient1_sampled(r, c)(0) -
			...
			for (int r = 0; r < width; ++r) {
				for (int c = 0; c < width; ++c) {
					Vec2 g(image_and_gradient1_sampled(r, c)(0),
						image_and_gradient1_sampled(r, c)(1));
			...
			for (i = 0; i < max_iterations; ++i) {
				...
				if (rho <= 0) {
					...
					*y2 = new_y2;
					...
					notify(INFO) << "Error decreased, so accept update.";
				}
				...
				if (d.squaredNorm() < min_update_squared_distance) {
					...
					if (minimum_correlation < correlation) {
						notify(INFO) << "Successful track in " << (i + 1) << " iterations.";
						return true;
					}
