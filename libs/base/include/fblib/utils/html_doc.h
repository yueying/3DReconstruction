#ifndef FBLIB_UTILS_HTML_DOC_H
#define FBLIB_UTILS_HTML_DOC_H
...
namespace fblib
{
	namespace utils{
		...
		public:
			...
			void init(const std::string &graph_name, int W, int H)
			{
				reset();
				stream
					<< "\n"
					<< "<div id=\"" << graph_name << "\" class=\"jxgbox\" style=\"width:" << W << "px; height:" << H << "px;\"></div>\n"
					<< "<script type=\"text/javascript\">\n"
					<< "var board = JXG.JSXGraph.initBoard('" << graph_name << "', {boundingbox: [-10, 10, 10, -10], axis:true,showCopyright:false});\n"
					<< "board.suspendUpdate();\n";
			}
