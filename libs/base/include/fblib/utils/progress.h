#ifndef FBLIB_UTILS_IMAGE_PROGRESS_H_
#define FBLIB_UTILS_IMAGE_PROGRESS_H_
...
namespace fblib{
	namespace utils{
		...
		/**
		* @brief ControlProgressDisplay displays an appropriate indication of progress at an appropriate place in an appropriate form.
		* -- Class modeled from BOOST::PROGRESS::progress_display
		* Usage :
		* ControlProgressDisplay my_progress_bar( fileList.size() );
		* for (list<string>::const_iterator it = fileList.begin(); it != fileList.end(); ++it, ++my_progress_bar)
		* the ControlProgressDisplay::operator++ take in charge the display of the progression (***)
		* {
		...
		**/
		...
		class ControlProgressDisplay : public C_Progress
		{
		public:
			/** @brief Standard constructor
			  * @param expected_count The number of step of the process
			  ...
			  **/
			explicit ControlProgressDisplay(unsigned long ulExpected_count,
			...
			/** @brief Initializer of the ControlProgressDisplay class
			 * @param expected_count The number of step of the process
			 **/
...
#endif // FBLIB_UTILS_IMAGE_PROGRESS_H_
