 ...
 *
********************************************************************************/
#ifndef FBLIB_SYNCH_THREADS_H_
#define FBLIB_SYNCH_THREADS_H_

#include <fblib/base/link_pragmas.h>
#include <Windows.h>
#include <fblib/utils/fblib_stdint.h>
...
namespace fblib
{
	namespace synch
	{
		...
		*/
		...
		/** 返回当前线程的ID
		*/
		unsigned long BASE_IMPEXP getCurrentThreadId() FBLIB_NO_THROWS;

		/** 返回当前线程的handle
		*/
		TThreadHandle BASE_IMPEXP getCurrentThreadHandle() FBLIB_NO_THROWS;

		/** 显示关闭当前正在运行的线程，不要经常使用，最好在运行线程返回的时候使用
		*/
		void BASE_IMPEXP exitThread() FBLIB_NO_THROWS;
		...
		*/
		...
		/** 终止线性，删除最后所有资源
		*/
		void BASE_IMPEXP terminateThread(TThreadHandle &threadHandle) FBLIB_NO_THROWS;
		...
		*/
		void BASE_IMPEXP sleep(int time_ms) FBLIB_NO_THROWS;
