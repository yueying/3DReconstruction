 ...
 *
********************************************************************************/
#ifndef FBLIB_THREADS_BLOCK_H_
#define FBLIB_THREADS_BLOCK_H_
	...
	namespace threads
	{
		...
		*/
		...
		public:
			...
			inline bool block()
			{
				ScopedLock<fblib::threads::Mutex> mutlock(m_mut);
				if (!m_released)
				{
		...
		*/
		...
		public:
			...
			{
				fblib::threads::ScopedLock<fblib::threads::Mutex> mutlock(m_mut);
				if (m_currentCount > 0)
				{
					--m_currentCount;
		...
		protected:
			...
			fblib::threads::Mutex m_mut;
			fblib::threads::Condition m_cond;
